...
- The redirect path never runs session, CSRF, authentication or message middleware. The one write it does make in the request is the click enqueue: in production the task backend is a database table, so enqueuing is a synchronous INSERT. That write is accepted — the alternative is a task queue this project does not have — and nothing else on this path should add a second one.
